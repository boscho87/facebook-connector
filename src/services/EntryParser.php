...
class EntryParser
{
    ...
    public function parseEntry($entry, $attachment)
    {
        ...
        $entry->title = $this->parseTitle($entry->title ?? '', $entry->content);
     ...
     * @return string
     */
    public function parseTitle(string $title, ?string $content): string
    {
        if (!$title) {
            $firstLineEnd = strpos($content, '<br');
            if ($firstLineEnd > 60 || $firstLineEnd === false) {
                $offset = strlen($content) > 60 ? 40 : 0;
                $firstLineEnd = strpos($content, ' ', $offset);
            }
            $title = substr($content, 0, $firstLineEnd);
        }
        $title = trim($title, '.');
        return trim($title) ?: '';
    }
    ...
    public function parseContent(string $content): string
    {
        ...
        return preg_replace('/u00\S+/', '', $content);
    }
     ...
     * @codeCoverageIgnore
     */
    private function ifYoutubeLink(?string $url): bool
    {
     ...
     * @return string
     */
    private function slugger(object $entry): string
    {
        ...
        $startPos = strpos($entry->fbId, '_');
        $name .= '-' . substr($entry->fbId, $startPos + 4, 6);
     ...
     * @return string
     */
    private function romeIToArabic(string $slug): string
    {
        return preg_replace_callback('~(\-(i+)\-?)$|(\-?(i+)\-)$|(-i+-)~', function ($value) {
            $value = $value[0];
            $countedI = substr_count($value, 'i');
            $value = preg_replace('~i+~', $countedI, $value);
